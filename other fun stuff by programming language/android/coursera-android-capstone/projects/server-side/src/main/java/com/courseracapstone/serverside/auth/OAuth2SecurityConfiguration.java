...
public class OAuth2SecurityConfiguration {
	...
	protected static class ResourceServer extends
		...
		public void configure(HttpSecurity http) throws Exception {
			...
			// allow requests to the signup api with just basic authentication
			http
			.authorizeRequests()
				.antMatchers(PotlatchConstants.POTLATCH_SIGN_UP_PATH).permitAll().and().httpBasic();
			...
			http
			.authorizeRequests()
				.antMatchers(HttpMethod.GET, "/potlatch/**")
				.access("#oauth2.hasScope('read')");
			
			// Require all other requests to have "write" scope
			http
			.authorizeRequests()
				.antMatchers("/potlatch/**")
				.access("#oauth2.hasScope('write')");

			// Require all secure requests to have "admin" scope
			http
			.authorizeRequests()
				.antMatchers("/potlatchadmin/**")
	...
	protected static class OAuth2Config extends
		...
		public OAuth2Config() throws Exception {
			...
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
					// Create a second privileged client that also has admin access
					.withClient(PotlatchConstants.ADMIN_CLIENT_ID).authorizedGrantTypes("password")
					.authorities("ROLE_CLIENT", "ROLE_ADMIN")
					.scopes("read","write","admin").resourceIds("potlatch", "potlatchadmin")
					.and()
					// Create a client that has normal user "read" and "write" access to the
			        // gifts service
					.withClient(PotlatchConstants.NORMAL_CLIENT_ID)
					.authorizedGrantTypes("password")
					.authorities("ROLE_CLIENT")
					.scopes("read","write")
					.resourceIds("potlatch")
					.and()
					// Create a client that has normal user "read" and "write" access to the
			        // gifts service
					.withClient(PotlatchConstants.NORMAL_CLIENT_ID)
...
//					.and()
			...
			// Create a series of hard-coded test users. 
			...
			for (int i = 0; i < PotlatchConstants.TEST_USER_NAMES.length; i++) {
				if (i == PotlatchConstants.ADMIN_TEST_USER_IDX) {
					users.add(User.create(PotlatchConstants.TEST_USER_NAMES[i], PotlatchConstants.TEST_USER_PWDS[i], "ADMIN", "USER"));
				}
				else {
					users.add(User.create(PotlatchConstants.TEST_USER_NAMES[i], PotlatchConstants.TEST_USER_PWDS[i], "USER"));
				}
			}
			
			UserDetailsService svc = MyInMemoryUserDetailsManager.newInstance(users);
