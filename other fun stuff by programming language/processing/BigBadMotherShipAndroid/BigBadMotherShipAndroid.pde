...
void setup() {
  ...
  maxAimLen = motherShipSize / getMaxAimLenMultiplier(); // motherShipSize is already dependent on window size
...
/**
 * combined the code from mouseClicked() at the start, and does the other mouseReleased() 
 * code only if gameControls doesn't handle it
 */
void mouseReleased() {
  ...
  else if (gameControls.isPlaying() && gameControls.isAiming()) {
...
void mouseClicked() {
  // doesn't seem to get called on android
}
