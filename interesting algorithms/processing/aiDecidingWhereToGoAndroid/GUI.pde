...
class RadioButtons extends Widget
{
  ...
  void setColours(color activeColour, color inactiveColour) {
    for (int i = 0; i < buttons.length; i++) {
      buttons[i].setActiveColor(activeColour);
      buttons[i].setInactiveColor(inactiveColour);
    }
  }
